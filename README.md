...
+   [tanh_quad](tanh_quad/tanh_quad.html)
    a library which
    sets up the tanh quadrature rule;
+   [tanh_sinh_rule](tanh_sinh_rule/tanh_sinh_rule.html)
    a program which
    computes and writes out a tanh-sinh quadrature rule of given order.
+   [task_division](task_division/task_division.html)
    a library which
...
+   [tec_write](tec_write/tec_write.html)
    a library which
...
+   [toms792](toms792/toms792.html)
    a library which
    tests functions that interpolate scattered data in the plane;
    by Robert Renka;
    this is a FORTRAN90 version of ACM TOMS algorithm 792.
+   [tri_surface_io](tri_surface_io/tri_surface_io.html)
    a library which
    reads and writes the 3D graphics information
    in a TRI_SURFACE file;
+   [triangle_exactness](triangle_exactness/triangle_exactness.html)
    a program which
    tests the polynomial exactness of a quadrature rule for triangles;
+   [triangle_grid](triangle_grid/triangle_grid.html)
    a library which
    computes a triangular grid of points.
+   [triangle_histogram](triangle_histogram/triangle_histogram.html)
    a program which
    computes histograms of data on the unit triangle.
+   [triangle_monte_carlo](triangle_monte_carlo/triangle_monte_carlo.html)
    a program which
    uses the Monte Carlo method to estimate an integral over a triangle.
+   [triangle_properties](triangle_properties/triangle_properties.html)
    a program which
    computes properties of a triangle
    whose vertex coordinates are read from a file.
+   [triangulation](triangulation/triangulation.html)
    a library which
    computes, plots and manipulates the triangulation of a set of
    points in 2D;
+   ["triangulation_boundary_nodes/triangulation_boundary_nodes.h])
...
+   [triangulation_corner](triangulation_corner/triangulation_corner.html)
    a program which
    modifies triangulations in which one or more triangles have more
    than one boundary edge.
+   [triangulation_delaunay_discrepancy](triangulation_delaunay_discrepancy/triangulation_delaunay_discrepancy.html)
    a program which
    measures the amount by which a triangulation fails the local Delaunay test;
+   [triangulation_histogram](triangulation_histogram/triangulation_histogram.html)
    a program which
    computes histograms of data over a triangulation.
+   [triangulation_l2q](triangulation_l2q/triangulation_l2q.html)
    a program which
...
+   [triangulation_quality](triangulation_quality/triangulation_quality.html)
    a program which
    reads information about a triangulation and computes various
    quality measures;
+   [triangulation_rcm](triangulation_rcm/triangulation_rcm.html)
    a program which
    computes the Reverse Cuthill-McKee (RCM) reordering for the nodes
    of a triangulation that uses 3-node or 6-node triangles;
+   [triangulation_refine](triangulation_refine/triangulation_refine.html)
    a program which
    refines a triangulation;
+   [triangulation_t3_to_t4](triangulation_t3_to_t4/triangulation_t3_to_t4.html)
    a program which
...
+   ["triangulation_triangle_neighbors/triangulation_triangle_neighbors.h])
...
+   [vandermonde_approx_2d](vandermonde_approx_2d/vandermonde_approx_2d.html)
    a library which
    finds a polynomial approximant p(x,y) to data z(x,y) of a 2D argument by setting up and
    solving an overdetermined linear system for the polynomial coefficients 
    involving the Vandermonde matrix.
+   [vandermonde_interp_1d](vandermonde_interp_1d/vandermonde_interp_1d.html)
    a library which
    ...
    involving the Vandermonde matrix.
+   [vandermonde_interp_2d](vandermonde_interp_2d/vandermonde_interp_2d.html)
    a library which
    finds a polynomial interpolant to data z(x,y) of a 2D argument by setting up and
    solving a linear system for the polynomial coefficients 
    involving the Vandermonde matrix.
+   [vec_io](vec_io/vec_io.html)
    a library which
    reads and writes vectors
    of fixed size, to and from a disk file, in any order;
+   [vector_plot](vector_plot/vector_plot.html)
    a program which
    creates a PostScript (PS) graphics file
    containing an image of a vector field (U(X,Y),V(X,Y));
+   [voronoi_plot](voronoi_plot/voronoi_plot.html)
    a program which
    plots the Voronoi neighborhoods of points in the 2D unit square,
    using L1, L2 or Loo norms;
+   [voronoi_weight](voronoi_weight/voronoi_weight.html)
    a program which
    estimates the volume of the Voronoi cells
    associated with N points in the M-dimensional unit hypercube;
+   [vtk_io](vtk_io/vtk_io.html)
    a library which
    reads and writes VTK graphics files.
+   [walsh](walsh/walsh.html)
    a library which
    implements versions of the Walsh and Haar transforms.
+   [wandzura](wandzura/wandzura.html)
    a library which
    returns one of 6 Wandzura rules for quadrature in a triangle;
+   [wavelet](wavelet/wavelet.html)
    a library which
    does some simple wavelet calculations;
+   [weekday](weekday/weekday.html)
    a library which
    determines the day of the week corresponding to a given date,
    such as 14 October 1066, Julian calendar, ...which was a Saturday.
+   [wordsnake](wordsnake/wordsnake.html)
    a program which
    rearranges a list of words so that they have maximum overlap;
+   [wtime](wtime/wtime.html)
    a library which
    returns a reading of the wall clock time in seconds.
+   [xerror](xerror/xerror.html)
    a library which
    forms the SLATEC error message package.
+   [xlf_intrinsics](xlf_intrinsics/xlf_intrinsics.html)
    programs which
    illustrate the use of some of the intrinsic functions
    included by the IBM XLF compiler, above and beyond those
    specified in the FORTRAN language standards.
+   [xy_io](xy_io/xy_io.html)
    a library which
    reads and writes XY files for storing 2D point information.
+   [xyz_io](xyz_io/xyz_io.html)
    a library which
    reads and writes XYZ files for storing 3D point information.
+   [xyz_plot](xyz_plot/xyz_plot.html)
    a program which
    is an interactive plotter.
+   [xyz_to_pdb](xyz_to_pdb/xyz_to_pdb.html)
    a program which
    reads an XYZ file of 3D spatial coordinates, and rewrites the information
    as atomic coordinates in a Protein Data Base (PDB) file;
+   [ziggurat](ziggurat/ziggurat.html)
    a library which
    samples the uniform, normal or exponential distributions,
    using the ziggurat method, 
    by Marsaglia and Tsang.
+   [ziggurat_openmp](ziggurat_openmp/ziggurat_openmp.html)
    a program which
    demonstrates how the ziggurat library can be used to generate random numbers
    in an OpenMP parallel program.
+   [zoomin](zoomin/zoomin.html)
    a library which
    finds the zero of a scalar function.
